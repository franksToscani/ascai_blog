    ...
    <form action="{{ route('admin.events.update', $event) }}" method="POST" class="bg-white rounded-xl shadow-sm p-6 space-y-4">
        ...
        <div>
            ...
            <textarea name="description" rows="5"
                class="w-full border border-slate-300 rounded px-3 py-2 text-sm">{{ old('description', $event->description) }}</textarea>
        </div>

        <div class="grid md:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium mb-1">Data/ora inizio</label>
                <input type="datetime-local" name="starts_at" value="{{ old('starts_at', $event->starts_at ? $event->starts_at->format('Y-m-d\TH:i') : '') }}"
                    class="w-full border border-slate-300 rounded px-3 py-2 text-sm">
            </div>
            <div>
                <label class="block text-sm font-medium mb-1">Data/ora fine (opzionale)</label>
                <input type="datetime-local" name="ends_at" value="{{ old('ends_at', $event->ends_at ? $event->ends_at->format('Y-m-d\TH:i') : '') }}"
                    class="w-full border border-slate-300 rounded px-3 py-2 text-sm">
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.events.index') }}" class="text-slate-600 text-sm underline">Annulla</a>
        </div>
    </form>
@endsection
